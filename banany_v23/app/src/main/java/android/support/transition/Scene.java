...
public class Scene {
    ...
    public static Scene getSceneForLayout(@NonNull ViewGroup sceneRoot, @LayoutRes int layoutId, @NonNull Context context) {
        SparseArray<Scene> scenes = (SparseArray) sceneRoot.getTag(C0076R.id.transition_scene_layoutid_cache);
        if (scenes == null) {
            scenes = new SparseArray();
            sceneRoot.setTag(C0076R.id.transition_scene_layoutid_cache, scenes);
        }
    ...
    static void setCurrentScene(View view, Scene scene) {
        view.setTag(C0076R.id.transition_current_scene, scene);
    }

    static Scene getCurrentScene(View view) {
        return (Scene) view.getTag(C0076R.id.transition_current_scene);
    }
