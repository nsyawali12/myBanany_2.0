...
class TypefaceCompatBaseImpl implements TypefaceCompatImpl {
    ...
    class C03551 implements StyleExtractor<FontInfo> {
        C03551() {
        }
    ...
    class C03562 implements StyleExtractor<FontFileResourceEntry> {
        C03562() {
        }
    ...
    private static <T> T findBestFont(T[] fonts, int style, StyleExtractor<T> extractor) {
        ...
        int targetWeight = (style & 1) == 0 ? NNTPReply.SERVICE_DISCONTINUED : 700;
        if ((style & 2) != 0) {
            isTargetItalic = true;
        } else {
    ...
    protected FontInfo findBestInfo(FontInfo[] fonts, int style) {
        return (FontInfo) findBestFont(fonts, style, new C03551());
    }
    ...
    private FontFileResourceEntry findBestEntry(FontFamilyFilesResourceEntry entry, int style) {
        return (FontFileResourceEntry) findBestFont(entry.getEntries(), style, new C03562());
    }
